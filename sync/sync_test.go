...
func TestSyncCache_SetAndGet(t *testing.T) {
	cache := NewSyncCache[string](time.Second)
...
func TestSyncCache_Expiration(t *testing.T) {
	cache := NewSyncCache[string](time.Millisecond * 10)
...
func TestSyncCache_GetWithoutCheck(t *testing.T) {
	cache := NewSyncCache[string](time.Second)
